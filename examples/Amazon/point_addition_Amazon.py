...
def main():
    ...
    parser.add_argument('--num', type=int, default=1000, help='number of samples to select')
    ...
    for test_domain in domains:
        ...
        for method in methods:
            ...
            set_seed()
            random_1000_idx = random.sample(list(method_df['indices'].unique()), 1000)

    
            
            selected_indices = random_1000_idx 

            if plus_n > 0:
                
                remaining_df = sorted_df[~sorted_df['indices'].isin(selected_indices)]
                additional_df = remaining_df.head(plus_n)

                additional_indices = additional_df['indices'].tolist()
                final_indices = selected_indices + additional_indices
            else:
            ...
            # Train and evaluate
            try:
                embeddings_selected = train_embeddings[final_indices]
                labels_selected = train_labels[final_indices]
