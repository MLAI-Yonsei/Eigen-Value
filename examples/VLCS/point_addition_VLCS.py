...
class VLCSDataset(Dataset):
    def __init__(self, label_files, data_root, transform=None):
        ...
        for idx, (path_, label_) in enumerate(tmp_list):
            ...
            else: # In case of truncated images
...
if __name__ == "__main__":
    ...
    parser.add_argument('--plus_n', type=int, default=50, help='for point addition, number of adding data points')
    ...
    for exclude_idx in range(4):
        ...
        damaged_indices = [504, 558, 1592, 1593, 1758,3933,5133]
        ...
        if len(selected_indices) < 2000:
            ...
            additional_needed = 2000 - len(selected_indices)
        ...
        selected_indices_2000 = selected_indices[:2000]


        
        train_dataset = Subset(raw_train, selected_indices_2000)
        ...
        fixed_train_loader = DataLoader(train_dataset, batch_size=16, shuffle=False, num_workers=0, collate_fn=skip_none_collate)
        
        
        val_remain_indices = selected_indices[2000:2500]

        val_subset = Subset(raw_train, val_remain_indices)
        ...
        train_order = {idx: pos for pos, idx in enumerate(all_train_indices)}
        ...
        for method in methods:
            ...
            if plus_n>0:
                remaining_method_df = method_df[ ~method_df['indices'].isin(sampled_idx) ]
                
                sorted_remaining_df = remaining_method_df.sort_values(by='data_values', ascending=False)
                additional_df = sorted_remaining_df.head(plus_n)
                additional_indices = additional_df['indices'].tolist()
                
                
                final_indices = sampled_idx + additional_indices
            else:
                final_indices = sampled_idx

            selected_positions = [train_order[idx] for idx in final_indices]
        ...
        output_dir = f'./point_addition/ViT/{excluded_domain}'
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        output_path = os.path.join(output_dir, f'performance_ascending_{ascending}_plus_n_{plus_n}.csv')
